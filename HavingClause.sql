...
--WHERE {Filtering Condition}GROUP BY [ColumnName]
--HAVING [Aggregate Function] {Comparison operator} [filtering criteria]
--ORDER BY {ColumnName, Ordinal, Alias}
...
SELECT Department AS [Department Name], COUNT(*) AS [Employee Count] FROM HumanResources.vEmployeeDepartment
GROUP BY Department HAVING COUNT(*) IN (6,10)

SELECT SalesPersonID, SUM(TotalDue) AS [Total Sales Amount], COUNT(*) AS [Total Sales Count]
 FROM Sales.SalesOrderHeader WHERE OrderDate BETWEEN '1/1/2006' AND '12/31/2006' AND SalesPersonID IS NOT NULL
